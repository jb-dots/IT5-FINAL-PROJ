...
#--------------logo - --------------------
        ...
        #-------------title--------------------
        lbl_title=Label(self.root,text="ROOM DETAILS",font=("times new roman",18,"bold"),bg="#89b0a4", fg="#f7e7ce", bd=4, relief=RIDGE)
...
#room type
        ...
        entryroomtype=ttk.Combobox(lblframeleft,textvariable=self.vartype,font=("times new roman",13,"bold"),width=18, state="readonly")
...
#-------------------btns------------------
        ...
        buttonframe.place(x=20,y=180,width=412,height=40)
        ...
        buttonreset.grid(row=0, column=3, padx=2)
...
#----------------table frame search system-------------
        ...
        Tableframe.place(x=500,y=50,width=600,height=400)
    ...
    def add_data(self):
        try:
            ...
            if self.varfloor.get() == "" or self.varroom.get() == "" or self.vartype.get() == "":
                messagebox.showerror("Error", "All fields are required", parent=self.root)

            # Check if 'floor' and 'room number' are integers
            elif not self.varfloor.get().isdigit() or not self.varroom.get().isdigit():
                messagebox.showerror("Error", "Floor and Room Number must be integers", parent=self.root)

            elif int(self.varfloor.get())< 0 or int(self.varroom.get())< 0:
                messagebox.showerror("Error", "Floor and Room Number must be positive", parent=self.root)
            else:
                # If everything is valid, proceed with the database insertion
                try:
                    conn = mysql.connector.connect(host="localhost", user="root", password="",
                                                   database="hotel_management_system")
                    mycursor = conn.cursor()
                    mycursor.execute("INSERT INTO details (floor, roomno, roomtype) VALUES (%s, %s, %s)", (
                        int(self.varfloor.get()),  # Convert to integer before insertion
                        int(self.varroom.get()),  # Convert to integer before insertion
                        self.vartype.get()  # Convert to integer before insertion
                    ))
                    conn.commit()
            ...
            messagebox.showerror("Error",
                                 "Invalid input. Please enter valid numbers for Floor, Room Number, and Room Type.",
                                 parent=self.root)

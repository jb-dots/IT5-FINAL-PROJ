...
class Roombooking:
    def __init__(self, root):
        ...
        comboRoomAvailable = ttk.Combobox(labelframeleft, textvariable = self.var_roomavailable, font=("arial", 13, "bold"), width=27, state="readonly")
        comboRoomAvailable["value"]=("Occupied","Vacant")
    ...
    def add_data(self):
        ...
        elif not self.var_noofdays.get().isdigit():
            messagebox.showerror("Error", "Please enter a valid number.", parent=self.root)

        elif int(self.var_noofdays.get()) < 0:
            messagebox.showerror("Error", "Room available and number of days cannot be negative.", parent=self.root)
    ...
    def update(self):
        if self.var_contact.get() == "":
            ...
            messagebox.showerror("Error", "Please enter valid numbers", parent=self.root)

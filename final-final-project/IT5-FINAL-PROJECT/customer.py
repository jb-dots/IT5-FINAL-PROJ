...
class Cust_Win:
    def __init__(self, root):
        ...
        btnSearch = Button(Table_Frame, text="SEARCH", command= self.search_data, font=("times new roman", 12, "bold"), bg="#0F3325", fg="white", width=10)
        btnSearch.grid(row=0, column=3, padx=1)

        btnShowAll = Button(Table_Frame, text="SHOW ALL",command=self.fetch_data, font=("times new roman", 12, "bold"), bg="#0F3325", fg="white", width=10)
    ...
    def add_data(self):
        ...
        elif not self.txt_postcode.get().isdigit() or not self.txt_idnumber.get().isdigit():
            messagebox.showerror("Invalid Input", "Postcode and ID Number must be numeric.")

        elif int(self.entry_ref.get()) <0:
            messagebox.showerror("Invalid input", "ref does not accept negative values")
    ...
    def update_data(self):
        if self.entry_ref.get() == "" or self.txtcname.get() == "":
            ...
            try:
                ...
                my_cursor = conn.cursor()
                my_cursor.execute(
                    "UPDATE customer SET name=%s, gender=%s, postcode=%s, mobile=%s, email=%s, nationality=%s, idproof=%s, idnumber=%s, address=%s WHERE ref=%s",
                    (
                        self.txtcname.get(),
                        self.combo_gender.get(),
                        self.txt_postcode.get(),
    ...
    def delete_data(self):
        if self.entry_ref.get() == "":
            ...
            try:
                ...
                self.reset()  # Clear the form after deletion
                messagebox.showinfo("Delete", "Customer details have been deleted.")
            except mysql.connector.Error as e:
                messagebox.showerror("Error", f"Database Error: {e}")
        ...
        if self.txt_search.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host="localhost", username="root", password="",
                                               database="hotel_management_system")
                my_cursor = conn.cursor()
                search_query = "SELECT * FROM customer WHERE " + str(self.search_var.get()) + " LIKE '%" + str(
                    self.txt_search.get()) + "%'"
